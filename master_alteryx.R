...
noncb_forecast_data        <- function(period){ 
  ...
  ncb_fct_raw_data <- consolidation(forecast_resources(period, target = "NonCB")) %>% 
    ...
    group_by(team,growth_initiative,
             account_l1,account_l2,account,
             month, month_num, quarter) %>%
    summarise(forecast = sum(value),.groups = "drop") %>%
    replace_na(list(forecast = 0)) %>%
...
cb_forecast_data           <- function(period){ 
  ...
  cb_fct_raw_data <- consolidation(forecast_resources(period, target = "CB")) %>% 
    ...
    pivot_longer(!c(team, growth_initiative, brassring_job_title,
                    brassring_req_number,employee_name,employee_id,
                    country,budgeted_salary_usd,forecasted_salary_usd,
                    bene_taxes_percent_formula,budgeted_start_month,
                    forecasted_start_date,hiring_status,technical_skillset_gained_with_role,
                    employee_level,backfill_name_if_applicable,cost_center_global,
                    hiring_manager, variable_comp_if_applicable,scenario),
                 names_to = "month", values_to = "value") %>%
    filter(!grepl("q|fy",month)) %>% 
    mutate(month = str_to_title(month)) %>%
    ...
    replace_na(list(value = 0)) %>% 
    group_by(team,growth_initiative,
             brassring_job_title,brassring_req_number,
             employee_name, employee_id, country, budgeted_salary_usd,
             bene_taxes_percent_formula, budgeted_start_month, forecasted_start_date,
             hiring_status,hiring_manager, technical_skillset_gained_with_role,
             employee_level, backfill_name_if_applicable, cost_center_global,
             account_l1,account_l2,account, variable_comp_if_applicable,
             month, month_num, quarter) %>%
    summarise(forecast = sum(value),.groups = "drop") %>%  
    ungroup() %>% 
    ...
    filter(!is.na(growth_initiative)) %>%
    mutate(team = ifelse(is.na(team),"NA",team)) %>% 
    mutate(ftes_started = ifelse(forecast > 0,1,0)) %>% 
    group_by(growth_initiative, team, country, 
             account, account_l1, account_l2, 
             month, month_num, quarter, hiring_status) %>%
    summarise(forecast = sum(forecast),
              forecasted_ftes = sum(ftes_started),
              .groups = "drop") %>% 
    ungroup()
...
# Capex Query 
capex_forecast_data        <- function(period){ 
  
  
  # Data Wrangling for NonCB
  read_forecast_capex <- function(file, sheet){ 
  ...
  capex_fct_raw_data <- consolidation(forecast_resources(period, target = "Capex")) %>%  
    filter(!is.na(far_number)|
           !is.na(far_description)|
           !is.na(internal_order_number)|
           !is.na(internal_order_description)|
    ...
    separate(file_name, c("file_name1","file_name2",
                          "file_name3","file_name4"),sep = "([/])") %>%
    select(-file_name1, -sheet_name,
           -file_name4, -spend_category,
           -spend_description, -cost_center_local,
           -vendor, -purchase_order_number, -erp_system, 
           -cost_center_global, -contact) %>%
    rename(scenario = file_name2,
           date = file_name3) %>%
    select(-date) %>%  
    pivot_longer(!c(team, growth_initiative,
    ...
    group_by(team, growth_initiative,
             ...
             account, account_l1,account_l2,
             month,month_num, quarter) %>%
    summarise(forecast = sum(as.numeric(value)),
              .groups = "drop") %>%
    ungroup() %>% 
  ...
  return(capex_fct_raw_data)
  
}                    # ✔✔✔
# Capex Forecast, inputs;       (p = month.#, as.number())
